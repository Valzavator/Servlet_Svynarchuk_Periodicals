...
public class ConnectionProxy implements Connection {
    ...
    void incrementAccessCounter() {
        accessCounter++;
        LOGGER.debug("ConnectionProxy counter was incremented. Current value: {}", accessCounter);
    }
    ...
    public void close() {
        ...
        if (accessCounter == 0) {
            try {
                connection.close();
                LOGGER.debug("Closing connection success");
            } catch (SQLException e) {
                LOGGER.error("Closing connection fail", e);
            } finally {
                dataSourceProxy.removeConnection();
            }
        } else {
            LOGGER.debug("ConnectionProxy counter was decremented. Current value: {}", accessCounter);
        }
    }
